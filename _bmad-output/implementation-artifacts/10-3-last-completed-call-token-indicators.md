...
1. **Last completed call state**
   ...
   - A later valid `token_usage` event replaces the previous displayed call values, including when several LLM calls occur in one user turn.
   - Invalid, unrelated, or unknown custom events are ignored without changing the last valid usage.
   - Starting, loading, or resetting a conversation clears the last-call usage.
...
2. **Context indicator**
   ...
   - The indicator identifies the value as context usage for the **last completed call**, and displays the formatted input count and percentage.
...
5. **Completed-call output details**
   - Given `output_tokens` and a positive `provider_max_output_tokens` are available in the last completed-call event, the UI presents a compact secondary output count and percentage.
   ...
   - The output percentage uses the backend-provided `output_ratio` and is not estimated or recomputed from rounded values.

6. **Usage details and omissions**
   - An accessible details affordance exposes every available value from the last valid call: input, output, context limit, provider output limit, cache-read, cache-write, and reasoning tokens.
...
7. **Authoritative cumulative usage**
   ...
   - Multiple `token_usage` events in one turn therefore replace the last-call display but do not increase the cumulative total more than once.
   - A complete turn and an interrupted/iteration-limit turn both reconcile from their single authoritative `turn_usage` event.
   - The cumulative indicator exposes the confirmed cumulative counts available from the stream and omits fields that have never been confirmed.
...
- [ ] Task 1: Define validated frontend usage contracts (AC: #1, #4, #5, #6, #7)
  - [ ] Add typed usage/domain shapes for per-call and per-turn values using the existing TypeScript conventions.
  - [ ] Add Zod schemas or equivalent runtime guards at the AG-UI custom-event boundary for `token_usage` and `turn_usage`.
  - [ ] Validate `event.name` and `event.value` from `@ag-ui/core` `CustomEvent`; the protocol value is `z.ZodAny`/unknown and must not be trusted as a typed object.
  - [ ] Accept only finite, non-negative integer token counts and finite non-negative ratios/limits; omit unavailable fields instead of coercing malformed values to zero.
  - [ ] Preserve the backend field names: `model`, `input_tokens`, `output_tokens`, `cache_read_tokens`, `cache_write_tokens`, `reasoning_tokens`, `context_window_tokens`, `provider_max_output_tokens`, `context_ratio`, and `output_ratio`.
  - [ ] Keep `turn_usage` counts-only; do not require per-call limits or ratios for reconciliation.

- [ ] Task 2: Ingest events through the AG-UI agent subscription (AC: #1, #7, #8)
  - [ ] Extend `ChatUIProvider` state and effects to subscribe to `agent.subscribe({ onCustomEvent })` from `useAgent()`.
  - [ ] Handle only validated `token_usage` and `turn_usage` events; leave all existing CopilotKit error subscription behavior intact.
  - [ ] Replace last-call usage on every valid `token_usage` event, including repeated events in one turn.
  - [ ] Add only valid `turn_usage` totals to cumulative state; do not maintain a client-side per-call scratch sum as the source of truth.
  - [ ] Return the subscription cleanup from the React effect and ensure the callback cannot mutate state after the agent/conversation changes.
  - [ ] Reset both usage states on the existing conversation reset signal and on agent/thread identity changes. Do not clear usage merely because a response is currently running.

- [ ] Task 3: Expose state through the UI context (AC: #1, #7, #9)
  - [ ] Extend `chat-ui-context-types.ts` and the context value with last-call usage, cumulative usage, and any derived status/detail data needed by presentation components.
  - [ ] Use stable, explicit types; do not add `any`, broad casts, or duplicate conversation state.
  - [ ] Keep provider callbacks and existing context consumers backward compatible.

- [ ] Task 4: Build the compact indicator and details UI (AC: #2, #3, #4, #5, #6, #9)
  - [ ] Add a focused presentation component under `src/components` (for example `TokenUsageIndicator.tsx`) rather than embedding parsing or aggregation logic in `ChatView`.
  - [ ] Render it adjacent to `ModelSelector` in the existing model-controls area, preserving the current responsive layout and controls.
  - [ ] Render normal/warning/critical/blocked context states with both visual styling and accessible status text.
  - [ ] Render the unavailable-limit state without a percentage or progress value.
  - [ ] Render completed output details only when confirmed output data exists; never bind the output display to `agent.isRunning` as a live meter.
  - [ ] Provide an accessible details disclosure/popover using existing project primitives and semantics; do not add a UI library solely for this story.
  - [ ] Ensure long counts, labels, and details remain usable at narrow viewport widths.

- [ ] Task 5: Add provider and component tests (AC: #1-#9)
  - [ ] Extend `src/__tests__/chat-ui-context.test.tsx` with valid event ingestion, replacement of repeated per-call events, invalid payload rejection, authoritative cumulative reconciliation, interrupted-turn totals, cleanup, and reset cases.
  - [ ] Extend `src/__tests__/chat-view.test.tsx` or add a focused indicator test with placement, all four thresholds, unavailable-limit behavior, output details, omissions, accessibility text/roles, and narrow-layout-safe rendering.
  - [ ] Assert that two per-call events plus one matching `turn_usage` event produce one cumulative addition, not two.
  - [ ] Assert that reset clears both indicators and that a stale/unsubscribed agent cannot update the new conversation.
  - [ ] Use the repository's existing Vitest and Testing Library mocks; keep tests deterministic and dependency-free.

- [ ] Task 6: Validate the frontend (AC: #9)
  - [ ] Run the existing formatter/linter/type-check/build commands defined by `talk-ui/package.json`.
  - [ ] Run the focused context and component tests, then the existing frontend test suite if the focused tests reveal shared regressions.
  - [ ] Do not install dependencies or alter package versions unless an existing command proves a dependency is missing.
...
- Do not “fix” or reinterpret backend omissions in the UI.
...
| Output | Completed count/ratio only; no live meter while `agent.isRunning`; absent output/limit omitted |
...
- [Source: `_bmad-output/planning-artifacts/epics.md` — Epic 10, Story 10.3, frontend FR-1 to FR-14]
- [Source: `_bmad-output/planning-artifacts/prds/prd-token-observability-frontend-2026-09-05/prd.md` — functional requirements, thresholds, accessibility, reset, cumulative usage]
- [Source: `_bmad-output/planning-artifacts/prds/prd-token-observability-frontend-2026-09-05/addendum.md` — compact horizontal indicator and last-completed-call semantics]
- [Source: `_bmad-output/planning-artifacts/prds/prd-token-observability-frontend-2026-09-05/.decision-log.md` — placement and update decisions]
- [Source: `_bmad-output/planning-artifacts/prds/prd-token-observability-backend-2026-09-05/addendum.md` — event payload and authoritative `turn_usage` reconciliation]
- [Source: `_bmad-output/implementation-artifacts/10-2-per-response-token-usage-in-the-ag-ui-stream.md` — backend event contract and omission rules]
- [Source: `talk-ui/src/context/ChatUIContext.tsx` — provider state and agent lifecycle]
- [Source: `talk-ui/src/context/chat-ui-context-types.ts` — context public contract]
- [Source: `talk-ui/src/components/ChatView.tsx` — model-controls placement]
- [Source: `talk-ui/src/components/ModelSelector.tsx` — accessible model selector]
- [Source: `talk-ui/src/config/agui-schemas.ts` — existing AG-UI validation patterns]
- [Source: `talk-ui/src/map/MapProvider.tsx` — existing empty-message reset precedent]
- [Source: `talk-ui/node_modules/.pnpm/@ag-ui+core@0.0.57/node_modules/@ag-ui/core/dist/index.d.ts` — `CustomEvent` shape]
- [Source: `talk-ui/node_modules/@ag-ui/client/dist/index.d.ts` — `AbstractAgent.subscribe` and `AgentSubscriber.onCustomEvent`]
